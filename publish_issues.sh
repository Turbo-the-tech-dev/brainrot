...
ls *.html >/dev/null 2>&1 && (grep -rq "Content-Security-Policy" --exclude-dir=.git --exclude-dir=media --exclude-dir=archives --exclude-dir=node_modules . || missing=true) || missing=false
...
# 40. Hardcoded URLs
grep -r "http://" --exclude-dir=.git --exclude-dir=media --exclude-dir=archives --exclude-dir=node_modules . | grep -q "." && missing=true || missing=false
