...
ls *.html >/dev/null 2>&1 && (grep -rq --exclude-dir=.git --exclude-dir=media --exclude-dir=archives --exclude-dir=node_modules "Content-Security-Policy" . || missing=true) || missing=false
...
# 34. Large File Bloat
find . -type d \( -name .git -o -name media -o -name archives -o -name node_modules \) -prune -o -type f -size +1M -print | grep -q "." && missing=true || missing=false
