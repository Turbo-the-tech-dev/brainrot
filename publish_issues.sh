...
# Bolt: Exclude large/irrelevant dirs for faster recursive search
...
# 40. Hardcoded URLs
# Bolt: Exclude large/irrelevant dirs for faster recursive search
grep -r --exclude-dir={.git,node_modules,media,archives} "http://" . | grep -v "node_modules" | grep -q "." && missing=true || missing=false
