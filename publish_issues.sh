...
ls *.html >/dev/null 2>&1 && (grep -rq --exclude-dir=.git --exclude-dir=media --exclude-dir=archives "Content-Security-Policy" . || missing=true) || missing=false
...
# 34. Large File Bloat
# Optimized: Prune large known-heavy directories to speed up the search
find . -type d \( -name .git -o -name node_modules \) -prune -o -type f -size +1M -print | grep -q "." && missing=true || missing=false
...
# 40. Hardcoded URLs
# Optimized: Exclude large directories and use built-in exclusion for node_modules
grep -r --exclude-dir=.git --exclude-dir=media --exclude-dir=archives --exclude-dir=node_modules "http://" . | grep -q "." && missing=true || missing=false
