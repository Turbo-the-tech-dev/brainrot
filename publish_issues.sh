...
# Performance: Excluding large non-source directories from recursive search
ls *.html >/dev/null 2>&1 && (grep --exclude-dir=.git --exclude-dir=node_modules --exclude-dir=media --exclude-dir=archives -rq "Content-Security-Policy" . || missing=true) || missing=false
...
# 34. Large File Bloat
# Performance: Pruning .git and node_modules from search to avoid known large files
...
# 40. Hardcoded URLs
# Performance: Excluding large non-source directories from recursive search
grep --exclude-dir=.git --exclude-dir=node_modules --exclude-dir=media --exclude-dir=archives -r "http://" . | grep -v "node_modules" | grep -q "." && missing=true || missing=false
