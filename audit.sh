...
grep -riq --exclude-dir={.git,archives,media,node_modules} "roadmap" README.md . >/dev/null 2>&1 && missing=false || missing=true
...
# Optimization: Exclude large binary/backup directories from recursive search
grep -r --exclude-dir={.git,archives,media,node_modules} "process.env" . >/dev/null 2>&1 && [ ! -f .env.example ] && missing=true || missing=false
...
# Optimization: Exclude large binary/backup directories from recursive search
grep -riq --exclude-dir={.git,archives,media,node_modules} "app.get" . >/dev/null 2>&1 && [ ! -f openapi.yaml ] && missing=true || missing=false
