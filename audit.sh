...
# 9. Missing RoadMap
# Optimized: Exclude large directories from recursive grep and check README first
grep -riq --exclude-dir=.git --exclude-dir=media --exclude-dir=archives --exclude-dir=node_modules "roadmap" README.md . >/dev/null 2>&1 && missing=false || missing=true
...
# Optimized: Exclude large directories from recursive grep
grep -r --exclude-dir=.git --exclude-dir=media --exclude-dir=archives --exclude-dir=node_modules "process.env" . >/dev/null 2>&1 && [ ! -f .env.example ] && missing=true || missing=false
...
# Optimized: Exclude large directories from recursive grep
grep -riq --exclude-dir=.git --exclude-dir=media --exclude-dir=archives --exclude-dir=node_modules "app.get" . >/dev/null 2>&1 && [ ! -f openapi.yaml ] && missing=true || missing=false
