...
grep --exclude-dir=.git --exclude-dir=node_modules --exclude-dir=media --exclude-dir=archives -riq "roadmap" README.md . >/dev/null 2>&1 && missing=false || missing=true
...
# Performance: Excluding large non-source directories from recursive search
grep --exclude-dir=.git --exclude-dir=node_modules --exclude-dir=media --exclude-dir=archives -r "process.env" . >/dev/null 2>&1 && [ ! -f .env.example ] && missing=true || missing=false
...
# Performance: Excluding large non-source directories from recursive search
grep --exclude-dir=.git --exclude-dir=node_modules --exclude-dir=media --exclude-dir=archives -riq "app.get" . >/dev/null 2>&1 && [ ! -f openapi.yaml ] && missing=true || missing=false
