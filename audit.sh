#!/bin/bash
# Performance Optimization: Optimized recursive grep calls to skip large non-source directories./bin/bash
...
# 9. Missing RoadMap
grep --exclude-dir={.git,node_modules,media,archives} -riq "roadmap" . >/dev/null 2>&1 && missing=false || missing=true
...
grep --exclude-dir={.git,node_modules,media,archives} -riq "reproduce" .github/ISSUE_TEMPLATE/*.md >/dev/null 2>&1 && missing=false || missing=true
