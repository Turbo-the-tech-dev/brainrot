...
# SECURITY GATE: Reject input with shell metacharacters to prevent command injection
if echo "$LAST_THOUGHT" | grep -qE '`|\$\('; then
    echo ">> [ERROR] SECURITY VIOLATION: Malicious neural thought detected. Aborting."
    exit 1
fi
